...
# Study Drills
# ------------
# 1. raw_input() takes input from the user.  Optionally, you can specify a prompt
# raw_input('--->') prints '--->' as a prompt to the user.
...
print "\n"
print "So you've had %s beers and you said \'%s\' to another." % (beers, answer)
print "\n"
# 4. In the original code we used %r which is the repr() format string in python 
# it prints out the strings the way python sees them - i.e. with quotes and since the 
# single quote character is used as short hand for "feet" it would look to python as 
# the end of the string so it needs to be escaped.  This is fixed in the second set of 
# code which uses the format string %s